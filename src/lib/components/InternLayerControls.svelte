...
<div class="marker-filter-container" transition:fade>
	...
	/>
	<label for={'styled-checkbox-labels'} style:background-color={'white'} style:color={'black'}
		>Toon naam samenwerkingen</label
	>
	<input
	class="styled-checkbox"
...
/>
<label for={'styled-checkbox-namen'} style:background-color={'white'} style:color={'black'}
	>Toon gemeentenamen</label
>

<GemeenteSearch bind:activeGemeente bind:showInfo {gemeenten}></GemeenteSearch>
	...
	<ul class="unstyled centered">
		{#each Object.keys(selectedCoops) as laag}
			<li>
				...
				/>
				<label for={'styled-checkbox-' + laag} style:background-color={colorScale[laag]}
					>
					{#if laag == 'igs' || laag == 'regio'}
						{laag}
					{/if}
					{#if laag != 'igs' && laag != 'regio'}
					{laag} <svg width="50px" height="20px" style:float={'right'}>
						<defs>
